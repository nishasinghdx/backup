...
function course_completion_dates_month($courseid){
global $DB;
$time=strtotime("-10 week");
$course_completion_dates_month=$DB->get_records_sql("SELECT timemodified as course_completion_dates_month
  FROM {course_modules_completion} cmc
  LEFT OUTER JOIN {course_modules} cm ON cmc.coursemoduleid=cm.id
  WHERE (timemodified>=$time) AND (cm.course=$courseid)
");
...
function count_users_completed_course_between_two_date($date1,$date2,$courseid){
  global $DB;
  $date1=$date1;
  $date2=$date2;
  $t1=strtotime($date1."".'00:00:00');
  $t2=strtotime($date2."".'24:00:00');
  $count_users_completed_course_between_two_date=$DB->get_record_sql("SELECT count(DISTINCT cmc.id) as count_user
  FROM {course_modules_completion} cmc
  LEFT OUTER JOIN {course_modules} cm ON cmc.coursemoduleid=cm.id
  WHERE (timemodified  BETWEEN $t1 AND $t2) AND (completionstate=1) AND (cm.course=$courseid)
  ");
  return $count_users_completed_course_between_two_date;
}

function course_completion_dates_week($courseid){
global $DB;
$time=strtotime("-1 week");
$course_completion_dates_week=$DB->get_records_sql("SELECT timemodified as course_completion_dates_week
  FROM {course_modules_completion} cmc
  LEFT OUTER JOIN {course_modules} cm ON cmc.coursemoduleid=cm.id
  WHERE (timemodified>=$time) AND (cm.course=$courseid)
");

foreach ($course_completion_dates_week as $value) {
  $course_completion_dates_week_arr[]=date('m/d/Y', $value->course_completion_dates_week);
}
$course_completion_dates_week_unique_arr=array_unique($course_completion_dates_week_arr);
return $course_completion_dates_week_unique_arr;
}
